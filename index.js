...
async function onStart() {
	...
	for (const b of storedTitles_id) {
		alltitles.set(b.title_id, b)
	}

	const storedTitles = await titles.findAll()
	for (const b of storedTitles) {
		alltitles.set(b.title_name, b)
	}

	const storedAdmins = await admins.findAll()
	for (const b of storedAdmins) {
		useradmins.set(b.user_id, b)
	}
}
...
async function addTitle(title_id, title_name) {
	...
	const newTitle = await titles.create({
		title_id: title_id,
...
vk.updates.on('message_new', async (context) => {
	...
	if (
		(context.text === 'tei!updatetable' &&
			getAdmins(context.senderId) &&
			context.createdAt === tdate + 1) ||
		context.createdAt === tdate
	) {
		const response = await fetch(
			`https://broadcast.vkforms.ru/api/v2/list/?token=${sender_token}`,
		)
		if (response.ok) {
			...
			for (const e of resultarray) {
				await addTitle(e.id, e.name)
			}

			context.send('Таблицы рассылок обновлены.')
		}
	} else if (
		context.text.includes('tei!addadmin') &&
		getAdmins(context.senderId) &&
		context.createdAt === tdate + 1
	) {
		const addtext = context.text
		...
		try {
			...
			const user_id = text[1].match(numberPattern)[0]
...
vk.updates.on('wall_post_new', async (context) => {
	...
	if (
		context.wall.createdAt === tdate + 1 ||
		context.wall.createdAt === tdate
	) {
		const response = await fetch(
			`https://broadcast.vkforms.ru/api/v2/list/?token=${sender_token}`,
		)
		...
		if (response.ok) {
			...
			for (const e of resultarray) {
				const shouldSkip = findSendTitle(e.name, context)
				if (shouldSkip) {
					return
				}
			}
